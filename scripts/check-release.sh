...
rg -q 'warden-state.sh.*on' "$ROOT/scripts/mqtt-control.sh"
rg -q 'CHROME_LIFECYCLE.*active' "$ROOT/scripts/warden-state.sh"
rg -q 'recover_staged' "$ROOT/scripts/warden-state.sh"
rg -q 'verify_or_rollback' "$ROOT/scripts/self-update.sh"
rg -q 'REDACTED' "$ROOT/scripts/create-diagnostics.sh"
rg -q 'screen_state.*OFF' "$ROOT/scripts/start-kiosk.sh"
