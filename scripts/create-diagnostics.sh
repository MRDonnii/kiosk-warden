...
capture services.txt systemctl --user --no-pager status kiosk-chrome.service kiosk-mqtt-control.service kiosk-health.service kiosk-watchdog.service kiosk-webui.service kiosk-vnc.service kiosk-novnc.service kiosk-capabilities.service kiosk-input-guardian.service kiosk-adaptive-brightness.service
capture journal.txt journalctl --user --no-pager -n 300 -u kiosk-chrome.service -u kiosk-mqtt-control.service -u kiosk-health.service -u kiosk-watchdog.service -u kiosk-webui.service -u kiosk-input-guardian.service -u kiosk-adaptive-brightness.service
...
[[ -f "$KIOSK_DIR/capabilities.json" ]] && cp "$KIOSK_DIR/capabilities.json" "$work/capabilities.json"
[[ -f "$KIOSK_DIR/profiles.json" ]] && cp "$KIOSK_DIR/profiles.json" "$work/profiles.json"
